...
namespace Hrtzz.BuildTools
{
    public class HrtzzBuildVersionWindow : EditorWindow
    {
        ...
        {
            settings = JsonUtility.FromJson<BuildVersionSettings>(System.IO.File.ReadAllText(HrtzzPerformBuildsEditor.VERSION_SETTINGS_LOCATION));
        ...
        private void OnGUI()
        {
            ...
            if (GUILayout.Button("Reset Build Settings", GUILayout.ExpandHeight(true)))
            {
                ...
                settings.v2 = 0;
                settings.v3 = 0;
                HrtzzPerformBuildsEditor.WriteBuildSettingToJson(settings);
                ShowVersionSettings();
            }
